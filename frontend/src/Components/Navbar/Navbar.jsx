...
export default function Navbar() {
  ...
  const hbUser = JSON.parse(localStorage.getItem("hbUser")) ;
  ...
  const handleLogout = () => {
    ...
    localStorage.setItem("hbUser", null);
    navigate("/");
  };
    ...
    <Box bg={useColorModeValue("gray.100", "gray.900")} px={1}>
      ...
      >
        <Box className="hamburger" display={{ md: "none" }}>
          {!down ? (
            <IconButton onClick={setDown.on} icon={<FaAlignLeft />} />
          ) : (
          ...
          <Stack direction={"row"} spacing={7}>
            ...
            <Menu>
              {hbToken ? (
                ...
                >
                  <ReactRouter to="/login">Login</ReactRouter>/
                  <ReactRouter to="/signup">Signup</ReactRouter>
                </Box>
              )}
